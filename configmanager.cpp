...
bool ConfigManager::load()
{
	...
	if(!m_loaded) //info that must be loaded one time- unless we reset the modules involved
	{
		...
		m_confString[MAP_NAME] = getGlobalString("mapName", "test.otbm");
	...
	m_confNumber[SPAWNPOS_X] = getGlobalNumber("newPlayerSpawnPosX", 1000);
	m_confNumber[SPAWNPOS_Y] = getGlobalNumber("newPlayerSpawnPosY", 1000);
