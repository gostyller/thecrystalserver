...
(\
	...
	FOREIGN KEY (`player_id`) REFERENCES `players` (`id`)\
		...
		case 24:
		{
			std::cout << ":: Updating database to version 25..." << std::endl;
			switch(db->getDatabaseEngine())
			{
				case DATABASE_ENGINE_SQLITE:
				{
		...
		case 25:
		{
			std::cout << ":: Updating database to version 26..." << std::endl;
			switch(db->getDatabaseEngine())
			{
				case DATABASE_ENGINE_SQLITE:
				{
		...
		case 26:
		{
			std::cout << ":: Updating database to version 27..." << std::endl;
		...
		case 27:
		{
			std::cout << ":: Updating database to version 28..." << std::endl;
			switch(db->getDatabaseEngine())
			{
				case DATABASE_ENGINE_SQLITE:
				{
				...
				case DATABASE_ENGINE_MYSQL:
				{
					...
					db->query("ALTER TABLE `guilds` ADD `balance` BIGINT UNSIGNED NOT NULL AFTER `motd`;");
					break;
				}
		...
		case 28:
		{
			std::cout << ":: Updating database to version 29..." << std::endl;
			switch(db->getDatabaseEngine())
			{
				case DATABASE_ENGINE_SQLITE:
				{
     			 	db->query("ALTER TABLE `accounts` ADD `vipdays` INTEGER NOT NULL DEFAULT '0';");
     			 	db->query("ALTER TABLE `accounts` ADD `lastvip` INTEGER NOT NULL DEFAULT '0';");
					 break;
    			}  

				case DATABASE_ENGINE_MYSQL:
				{
     			 	db->query("ALTER TABLE `accounts` ADD `vipdays` INT NOT NULL DEFAULT 0 AFTER `group_id`;");
     			 	db->query("ALTER TABLE `accounts` ADD `lastvip` INT NOT NULL DEFAULT 0 AFTER `vipdays`;");
					break;
				}
		...
		case 29:
		{
			...
			switch(db->getDatabaseEngine())
			{
				case DATABASE_ENGINE_SQLITE:
				{
					db->query("DROP TABLE `guild_kills`;");
					db->query("CREATE TABLE [guild_kills] \
					(\
		...
		if(newValue != (Encryption_t)value)
		{
			switch(newValue)
			{
				case ENCRYPTION_MD5:
				{
					...
					{
						std::cout << ":: WARNING: You cannot change the encryption to MD5, change it back in config.lua." << std::endl;
						return;
					}
				...
				case ENCRYPTION_SHA1:
				{
					if((Encryption_t)value != ENCRYPTION_PLAIN)
					{
						std::cout << ":: WARNING: You cannot change the encryption to SHA1, change it back in config.lua." << std::endl;
						return;
					}
				...
				case ENCRYPTION_SHA256:
				{
					if((Encryption_t)value != ENCRYPTION_PLAIN)
					{
						std::cout << ":: WARNING: You cannot change the encryption to SHA256, change it back in config.lua." << std::endl;
						return;
					}
					...
					registerDatabaseConfig("encryption", (int32_t)newValue);
					std::cout << ":: Encryption set to SHA256." << std::endl;
					break;
				}
				...
				case ENCRYPTION_SHA512:
				{
					if((Encryption_t)value != ENCRYPTION_PLAIN)
					{
						std::cout << ":: WARNING: You cannot change the encryption to SHA512, change it back in config.lua." << std::endl;
						return;
					}
					...
					registerDatabaseConfig("encryption", (int32_t)newValue);
					std::cout << ":: Encryption set to SHA512." << std::endl;
					break;
				}
				...
				case ENCRYPTION_VAHASH:
				{
					if((Encryption_t)value != ENCRYPTION_PLAIN)
					{
						std::cout << ":: WARNING: You cannot change the encryption to VAHash, change it back in config.lua." << std::endl;
						return;
					}
					...
					registerDatabaseConfig("encryption", (int32_t)newValue);
					std::cout << ":: Encryption set to VAHash." << std::endl;
					break;
				}
