...
bool DatabaseMySQL::connect(bool _reconnect)
{
	if(_reconnect)
	{
		std::cout << "WARNING: MYSQL Lost connection, attempting to reconnect..." << std::endl;
		if(++m_attempts > MAX_RECONNECT_ATTEMPTS)
		{
			std::cout << "Failed connection to database - maximum reconnect attempts passed." << std::endl;
			return false;
		}
	}
...
bool DatabaseMySQL::query(const std::string &query)
{
	...
	std::cout << "MYSQL DEBUG, query: " << query.c_str() << std::endl;
#endif
...
DBResult* DatabaseMySQL::storeQuery(const std::string &query)
{
	...
	int32_t error = 0;
#ifdef __SQL_QUERY_DEBUG__
	std::cout << "MYSQL DEBUG, storeQuery: " << query.c_str() << std::endl;
#endif
	if(mysql_real_query(&m_handle, query.c_str(), query.length()))
	{
...
void MySQLResult::free()
{
	if(!m_handle)
	{
		std::cout << "[Critical - MySQLResult::free] Trying to free already freed result!!!" << std::endl;
		return;
	}
