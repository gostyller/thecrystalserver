...
DatabaseSQLite::DatabaseSQLite()
{
	...
	{
		std::cout << "Failed to initialize SQLite connection: " << sqlite3_errmsg(m_handle) << " (" << sqlite3_errcode(m_handle) << ")" << std::endl;
...
bool DatabaseSQLite::query(const std::string& query)
{
	...
	std::cout << "SQLLITE DEBUG, query: " << buf << std::endl;
#endif
	sqlite3_stmt* stmt;
	// prepares statement
...
DBResult* DatabaseSQLite::storeQuery(const std::string& query)
{
	...
	std::string buf = _parse(query);
#ifdef __SQL_QUERY_DEBUG__
	std::cout << "SQLLITE DEBUG, storeQuery: " << buf << std::endl;
#endif
	sqlite3_stmt* stmt;
	// prepares statement
	if(OTSYS_SQLITE3_PREPARE(m_handle, buf.c_str(), buf.length(), &stmt, NULL) != SQLITE_OK)
...
void SQLiteResult::free()
{
	if(!m_handle)
	{
		std::cout << "[Critical - SQLiteResult::free] Trying to free already freed result!!!" << std::endl;
		return;
	}
