...
void Game::start(ServiceManager* servicer)
{
	...
	{
		std::cout << ":: WARNING: No valid hour (" << prepareHour << ") for a global save, should be between 0-23. Global save disabled." << std::endl;
		return;
	}

	if(prepareMinute < 0 || prepareMinute > 59)
	{
		std::cout << ":: WARNING: No valid minute (" << prepareMinute << ") for a global save, should be between 0-59. Global save disabled." << std::endl;
		return;
	}
	...
	if(gameState != newState)
	{
		...
		switch(newState)
		{
			case GAMESTATE_INIT:
			{
				...
				#ifdef __TCS_VIP__
				if(g_config.getBool(ConfigManager::INIT_VIP_UPDATE))
					IOManager::getInstance()->updateVipDays();
				#endif
...
void Game::saveGameState(bool shallow)
{
	...
	std::cout << ":: SAVE: Complete in " << (OTSYS_TIME() - start) / (1000.) << " seconds using "
...
{
	bool found = false;
	for(uint32_t i = 0; i < cmdStringVec.size() ;i++)
	{
		if(cmdStringVec[i] == tag)
		{
			found = true;
			break;
		}
...
bool Game::playerSayCommand(Player* player, SpeakClasses type, const std::string& text)
{
	...
	for(uint32_t i = 0; i < cmdStringVec.size(); i++)
	{
		if(cmdStringVec[i] == text.substr(0,1))
		{
...
bool Game::reloadInfo(ReloadInfo_t reload, uint32_t playerId/* = 0*/)
{
	...
	switch(reload)
	{
		...
		{
			if(g_commands.reload())
				done = true;
			else
				std::cout << "[Error - Game::reloadInfo] Failed to reload commands." << std::endl;
		...
		case RELOAD_MOUNTS:
		{
			if(Mounts::getInstance()->reload())
				...
				std::cout << "[Error - Game::reloadInfo] Failed to reload mounts." << std::endl;
...
{
	std::cout << "Game::prepareGlobalSave in " << (uint32_t)minutes << " minutes" << std::endl;
	switch(minutes)
	{
		case 5:
...
void Game::globalSave()
{
	...
	#ifdef __TCS_VIP__
	//remove vip days globaly if configured to
	if(g_config.getBool(ConfigManager::INIT_VIP_UPDATE))
		IOManager::getInstance()->updateVipDays();
	#endif
