...
Account IOManager::loadAccount(uint32_t accountId, bool preLoad/* = false*/)
{
	Account account;
	...
	#ifdef __TCS_VIP__
	query << "SELECT `name`, `password`, `salt`, `premdays`, `lastday`, `key`, `warnings`, `vipdays`, `lastvip` FROM `accounts` WHERE `id` = " << accountId << " LIMIT 1";
	#else
	...
	account.lastVip = result->getDataInt("lastvip");
	#endif
	...
	do
	{
		...
		if(GameServer* server = GameServers::getInstance()->getServerById(result->getDataInt("world_id")))
			...
			std::cout << "[Warning - IOManager::loadAccount] Invalid server for player '" << ss << "'." << std::endl;
#endif
	}
	while(result->next());
	result->free();
...
bool IOManager::saveAccount(Account account)
{
	Database* db = Database::getInstance();
	DBQuery query;
	#ifdef __TCS_VIP__
	query << "UPDATE `accounts` SET `premdays` = " << account.premiumDays << ", `warnings` = " << account.warnings << ", `lastday` = " << account.lastDay << ", `vipdays` = " << account.vipDays << ", `lastvip` = " << account.lastVip << " WHERE `id` = " << account.number << db->getUpdateLimiter();
	#else
...
bool IOManager::getAccountId(const std::string& name, uint32_t& number)
{
	if(!name.length())
		return false;
...
bool IOManager::getAccountName(uint32_t number, std::string& name)
{
	if(number < 2)
	{
		name = number ? "1" : "";
...
bool IOManager::hasFlag(uint32_t accountId, PlayerFlags value)
{
	Database* db = Database::getInstance();
	DBQuery query;
...
bool IOManager::hasCustomFlag(uint32_t accountId, PlayerCustomFlags value)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `group_id` FROM `accounts` WHERE `id` = " << accountId << " LIMIT 1";
...
bool IOManager::hasFlag(PlayerFlags value, const std::string& accName)
{
	Database* db = Database::getInstance();
	DBQuery query;
...
bool IOManager::hasCustomFlag(PlayerCustomFlags value, const std::string& accName)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `group_id` FROM `accounts` WHERE `name` " << db->getStringComparer() << db->escapeString(accName) << " LIMIT 1";
...
bool IOManager::accountIdExists(uint32_t accountId)
{
	Database* db = Database::getInstance();
	DBQuery query;
...
bool IOManager::accountNameExists(const std::string& name)
{
	Database* db = Database::getInstance();
	DBQuery query;
...
bool IOManager::getPassword(uint32_t accountId, std::string& password, std::string& salt, std::string name/* = ""*/)
{
	Database* db = Database::getInstance();
	DBQuery query;
...
bool IOManager::setPassword(uint32_t accountId, std::string newPassword)
{
	std::string salt;
...
bool IOManager::validRecoveryKey(uint32_t accountId, std::string recoveryKey)
{
...
bool IOManager::setRecoveryKey(uint32_t accountId, std::string newRecoveryKey)
{
...
uint64_t IOManager::createAccount(std::string name, std::string password)
{
...
void IOManager::removePremium(Account account)
{
...
#ifdef __TCS_VIP__
void IOManager::removeVipDays(Account account)
{
	uint64_t timeNow = time(NULL);
	if(account.vipDays > 0 && account.vipDays < 65535)
	{
		uint32_t days = (uint32_t)std::ceil((timeNow - account.lastVip) / 86400);
		if(days > 0)
		{
			if(account.vipDays >= days)
				account.vipDays -= days;
			else
				account.vipDays = 0;

			account.lastVip = timeNow;
		}
	}
	else
		account.lastVip = timeNow;
...
const Group* IOManager::getPlayerGroupByAccount(uint32_t accountId)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `group_id` FROM `accounts` WHERE `id` = " << accountId << " LIMIT 1";
...
bool IOManager::loadPlayer(Player* player, const std::string& name, bool preLoad /*= false*/)
{
	Database* db = Database::getInstance();
	DBQuery query;
			...
			if(pid >= 0 && pid < 100)
			{
				if(Container* c = item->getContainer())
				{
					if(Depot* depot = c->getDepot())
						...
						std::cout << "[Error - IOManager::loadPlayer] Cannot load depot " << pid << " for player " << name << std::endl;
				}
				else
					std::cout << "[Error - IOManager::loadPlayer] Cannot load depot " << pid << " for player " << name << std::endl;
			}
...
void IOManager::loadItems(ItemMap& itemMap, DBResult* result)
{
	do
	{
		uint64_t attrSize = 0;
		...
		if(Item* item = Item::CreateItem(result->getDataInt("itemtype"), result->getDataInt("count")))
		{
			if(!item->unserializeAttr(propStream))
				std::cout << "[Warning - IOManager::loadItems] Unserialize error for item with id " << item->getID() << std::endl;
...
bool IOManager::savePlayer(Player* player, bool preSave/* = true*/, bool shallow/* = false*/)
{
	...
	bool save = g_config.getNumber(ConfigManager::SAVE_GAME_DATA);
...
bool IOManager::saveItems(const Player* player, const ItemBlockList& itemList, DBInsert& query_insert)
{
	Database* db = Database::getInstance();
...
bool IOManager::playerDeath(Player* _player, const DeathList& dl)
{
	Database* db = Database::getInstance();
	DBQuery query;
...
bool IOManager::playerMail(Creature* actor, std::string name, uint32_t townId, Item* item)
{
	...
	if(player->isVirtual())
	{
		IOManager::getInstance()->savePlayer(player);
		delete player;
	}
...
bool IOManager::hasFlag(const std::string& name, PlayerFlags value)
{
	Database* db = Database::getInstance();
	DBQuery query;
...
bool IOManager::hasCustomFlag(const std::string& name, PlayerCustomFlags value)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `group_id` FROM `players` WHERE `name` " << db->getStringComparer() << db->escapeString(name) << " AND `deleted` = 0 LIMIT 1";
...
bool IOManager::hasFlag(PlayerFlags value, uint32_t guid)
{
	Database* db = Database::getInstance();
	DBQuery query;
...
bool IOManager::hasCustomFlag(PlayerCustomFlags value, uint32_t guid)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "SELECT `group_id` FROM `players` WHERE `id` = " << guid << " AND `deleted` = 0 LIMIT 1";
...
bool IOManager::isPremium(uint32_t guid)
{
...
#ifdef __TCS_VIP__
bool IOManager::isVip(uint32_t guid)
{
	if(g_config.getBool(ConfigManager::FREE_VIP))
		return true;
	...
	result->free();
	if(group && group->hasCustomFlag(PlayerFlag_IsAlwaysVipDays))
		return true;

	query.str("");
	query << "SELECT `vipdays` FROM `accounts` WHERE `id` = " << account << " LIMIT 1";
	if(!(result = db->storeQuery(query.str())))
		return false;

	const uint32_t vipd = result->getDataInt("vipdays");
	result->free();
	return vipd;
}
#endif

bool IOManager::playerExists(uint32_t guid, bool multiworld /*= false*/, bool checkCache /*= true*/)
{
	if(checkCache)
	{
...
bool IOManager::playerExists(std::string& name, bool multiworld /*= false*/, bool checkCache /*= true*/)
{
	if(checkCache)
	{
...
bool IOManager::getNameByGuid(uint32_t guid, std::string& name, bool multiworld /*= false*/)
{
	NameCacheMap::iterator it = nameCacheMap.find(guid);
	if(it != nameCacheMap.end())
	{
...
bool IOManager::storeNameByGuid(uint32_t guid)
{
	NameCacheMap::iterator it = nameCacheMap.find(guid);
	if(it != nameCacheMap.end())
		return true;
...
bool IOManager::getGuidByName(uint32_t& guid, std::string& name, bool multiworld /*= false*/)
{
	GuidCacheMap::iterator it = guidCacheMap.find(name);
	if(it != guidCacheMap.end())
	{
...
bool IOManager::getGuidByNameEx(uint32_t& guid, bool &specialVip, std::string& name)
{
	Database* db = Database::getInstance();
	DBQuery query;
...
bool IOManager::changeName(uint32_t guid, std::string newName, std::string oldName)
{
	Database* db = Database::getInstance();
	DBQuery query;
...
bool IOManager::createCharacter(uint32_t accountId, std::string characterName, int32_t vocationId, uint16_t sex)
{
...
DeleteCharacter_t IOManager::deleteCharacter(uint32_t accountId, const std::string& characterName)
{
...
uint32_t IOManager::getLevel(uint32_t guid) const
{
	Database* db = Database::getInstance();
	DBQuery query;
...
uint32_t IOManager::getLastIP(uint32_t guid) const
{
	Database* db = Database::getInstance();
	DBQuery query;
...
uint32_t IOManager::getLastIPByName(const std::string& name) const
{
	Database* db = Database::getInstance();
	DBQuery query;
...
uint32_t IOManager::getAccountIdByName(const std::string& name) const
{
	Database* db = Database::getInstance();
	DBQuery query;
...
bool IOManager::getUnjustifiedDates(uint32_t guid, std::vector<time_t>& dateList, time_t _time)
{
	Database* db = Database::getInstance();
	DBQuery query;
...
bool IOManager::getDefaultTownByName(const std::string& name, uint32_t& townId)
{
	Database* db = Database::getInstance();
	DBQuery query;
...
bool IOManager::updatePremiumDays()
{
	Database* db = Database::getInstance();
	DBQuery query;
...
bool IOManager::updateVipDays()
{
	...
	DBResult* result;
	query << "SELECT `id` FROM `accounts` WHERE `lastvip` <= " << time(NULL) - 86400;
	if(!(result = db->storeQuery(query.str())))
		return false;

	do
		removeVipDays(loadAccount(result->getDataInt("id"), true));
	while(result->next());
	result->free();
...
bool IOManager::updateOnlineStatus(uint32_t guid, bool login)
{
	Database* db = Database::getInstance();
	DBQuery query;
...
bool IOManager::resetGuildInformation(uint32_t guid)
{
	Database* db = Database::getInstance();
	DBQuery query;
