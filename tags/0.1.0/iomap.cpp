...
bool IOMap::loadMap(Map* map, const std::string& identifier)
{
	...
	if(headerMinorItems < CLIENT_VERSION_860)
	{
...
bool IOMap::loadMap(Map* map, const std::string& identifier)
{
	...
	std::cout << "[NOTE] Total tiles cached: " << cachedItems << "[+" << nullTiles << " blank tiles, " << testTiles-cachedItems << " uncached], saving ~" << (sizeof(Item) * cachedItems) / 1024.0 / 1024.0 << "MB ram" << std::endl;
	return true;
}
