...
void LuaInterface::executeTimer(uint32_t eventIndex)
{
	...
	if(it != m_timerEvents.end())
	{
		...
		if(reserveEnv())
		{
			...
			std::cout << "[Error - LuaInterface::executeTimer] Call stack overflow." << std::endl;
...
int32_t LuaInterface::internalGetPlayerInfo(lua_State* L, PlayerInfo_t info)
{
	...
	switch(info)
	{
		...
		case PlayerInfoPremiumDays:
			...
			break;
		#ifdef __TCS_VIP__
		case PlayerInfoVipDays:
			value = player->getVipDays();
			break; 
		#endif
		case PlayerInfoFood:
		{
...
{
	return internalGetPlayerInfo(L, PlayerInfoVipDays);
}
#endif
...
int32_t LuaInterface::luaDoPlayerAddVipDays(lua_State* L)
{
	...
	if(Player* player = env->getPlayerByUID(popNumber(L)))
	{
		if(player->vipDays < 65535)
		{
			Account account = IOManager::getInstance()->loadAccount(player->getAccount());
			if(days < 0)
			{
				account.vipDays = std::max((uint32_t)0, uint32_t(account.vipDays + (int32_t)days));
				player->vipDays = std::max((uint32_t)0, uint32_t(player->vipDays + (int32_t)days));
			}
			else
			{
				account.vipDays = std::min((uint32_t)65534, uint32_t(account.vipDays + (uint32_t)days));
				player->vipDays = std::min((uint32_t)65534, uint32_t(player->vipDays + (uint32_t)days));
			}
			IOManager::getInstance()->saveAccount(account);
		}
		lua_pushboolean(L, true);
