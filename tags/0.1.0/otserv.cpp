...
bool argumentsHandler(StringVec args)
{
	...
	for(StringVec::iterator it = args.begin(); it != args.end(); ++it)
	{
		...
		if((*it) == "--version" || (*it) == "-V")
		{
			std::cout << SOFTWARE_NAME << ", version " << SOFTWARE_VERSION << " (" << SOFTWARE_CODENAME << ")\n"
			"A server developed by Tryller.\n";
			return false;
		}
...
void otserv(StringVec, ServiceManager* services)
{
	...
	if(!getuid() || !geteuid())
	{
		std::cout << ":: WARNING: " << SOFTWARE_NAME << " has been executed as super user! It is "
	...
	std::cout << ":::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::" << std::endl;
	std::cout << ":: Crystal Server - Version 0.3 (Ice Fenix).                                 ::" << std::endl;
	...
	mutexName << "thecrystalserver_" << g_config.getNumber(ConfigManager::WORLD_ID);
	...
	else if(encryptionType == "sha512")
	{
		...
		std::cout << ":: Using SHA512 encryption" << std::endl;
	}
	else
	{
		g_config.setNumber(ConfigManager::ENCRYPTION, ENCRYPTION_PLAIN);
		std::cout << ":: Using plaintext encryption" << std::endl << std::endl
	...
	{
		std::cout << std::endl << ":: Calculating dmp1, dmq1 and iqmp for RSA...";
	...
	std::cout << ":: Loading items (XML)" << std::endl;
	...
	if(ip.size())
	{
		...
		std::cout << m_ip.to_string() << std::endl;
	}
	...
	if(!g_config.getBool(ConfigManager::BIND_ONLY_GLOBAL_ADDRESS))
	{
		...
		if(!gethostname(hostName, 128))
		{
			if(hostent* host = gethostbyname(hostName))
			{
				...
				if(s.str().size())
					std::cout << ":: Local IP address(es): " << s.str() << std::endl;
			}
		}
	...
	std::cout << std::endl << ":: Server has been loaded... starting up" << std::endl;
