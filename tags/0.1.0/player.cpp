...
void Player::__internalAddThing(uint32_t index, Thing* thing)
{
	...
	{
#ifdef __DEBUG_MOVESYS__
		std::cout << "Failure: [Player::__internalAddThing] index == 0 || index > 11" << std::endl;
#endif
		return;
	}
...
void Player::manageAccount(const std::string &text)
{
	...
	sendTextMessage(MSG_STATUS_CONSOLE_CYAN, msg.str().c_str());
	if(!noSwap)
		sendTextMessage(MSG_STATUS_CONSOLE_YELLOW, "Hint: Type 'account' to manage your account and if you want to start over then type 'cancel'.");
}
...
{
	if(g_config.getBool(ConfigManager::FREE_VIP) || hasCustomFlag(PlayerFlag_IsAlwaysVipDays))
		return true;
