...
bool ScriptManager::loadFromXml(const std::string& file, bool& enabled)
{
	...
	if(!doc)
	{
		std::cout << "[Error - ScriptManager::loadFromXml] Cannot load mod " << file << std::endl;
	...
	{
		std::cout << "[Error - ScriptManager::loadFromXml] Malformed mod " << file << std::endl;
		std::cout << getLastXMLError() << std::endl;
	...
	{
		std::cout << "[Warning - ScriptManager::loadFromXml] Empty name in mod " << file << std::endl;
		xmlFreeDoc(doc);
		return false;
	}
	...
	if(!supported)
	{
		std::cout << "[Warning - ScriptManager::loadFromXml] Your server is not supported by mod " << file << std::endl;
		xmlFreeDoc(doc);
		return false;
	}
	...
	if(mod.enabled)
	{
		...
		for(p = root->children; p; p = p->next)
		{
			...
			else if(!xmlStrcmp(p->name, (const xmlChar*)"lib") || !xmlStrcmp(p->name, (const xmlChar*)"config"))
			{
				...
				{
					std::cout << "[Warning - ScriptManager::loadFromXml] Lib without name in mod " << file << std::endl;
					continue;
				}
