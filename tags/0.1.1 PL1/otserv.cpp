...
bool argumentsHandler(StringVec args)
{
	...
	for(StringVec::iterator it = args.begin(); it != args.end(); ++it)
	{
		...
		if((*it) == "--version" || (*it) == "-V")
		{
			...
			"A server developed by Tryller, Mr.Ez.\n"
			"Visit our forum for updates, support and resources: http://vapus.net/.\n\n";;
			return false;
		}
...
void otserv(StringVec, ServiceManager* services)
{
	...
	std::clog << "::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::" << std::endl;
	std::clog << ":: Crystal Server - Version 0.1.1 PL1 (Ice Fenix).            ::" << std::endl;
	std::clog << ":: A server developed by Tryller, Mr.Ez.                      ::" << std::endl;
	std::clog << ":: Visit: http://vapus.net/                                   ::" << std::endl;
	std::clog << "::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::" << std::endl;
	...
	if(xmlDocPtr doc = xmlParseFile(VERSION_CHECK))
	{
		...
		if(!xmlStrcmp(root->name, (const xmlChar*)"versions"))
		{
			...
			if(!xmlStrcmp(p->name, (const xmlChar*)"entry"))
			{
				...
				if(tmp)
				{
					std::clog << "outdated, please update your server!" << std::endl;
					std::clog << ":: Current version information - version: " << SOFTWARE_VERSION << ", patch: " << VERSION_PATCH;
					std::clog << ", build: " << VERSION_BUILD << ", timestamp: " << VERSION_TIMESTAMP << "." << std::endl;
					std::clog << ":: Latest version information - version: " << version << ", patch: " << patch;
					std::clog << ", build: " << build << ", timestamp: " << timestamp << "." << std::endl;
	...
	std::clog << std::endl << ":: Server has been loaded... starting up" << std::endl;
