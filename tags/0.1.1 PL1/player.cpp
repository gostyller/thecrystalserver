	...
	Creature(), transferContainer(ITEM_LOCKER), name(_name), nameDescription(_name), client(p)
{
	...
	pzLocked = isConnecting = addAttackSkillPoint = requestedOutfit = mounted = false;
	saving = true;
...
std::string Player::getDescription(int32_t lookDistance) const
{
	...
	std::string tmp;
	if(marriage && IOManager::getInstance()->getNameByGuid(marriage, tmp))
	{
		s << ", ";
		if(vocationId == 0)
		{
...
void Player::onCreatureAppear(const Creature* creature)
{
	...
	if(!isGhost())
		IOManager::getInstance()->updateOnlineStatus(guid, true);
...
void Player::onChangeZone(ZoneType_t zone)
{
	if(zone == ZONE_PROTECTION)
	{
		...
		if(g_config.getBool(ConfigManager::DISMOUNT_PLAYER_IN_PZ))
			dismount(true);
	}
...
void Player::onCreatureDisappear(const Creature* creature, bool isLogout)
{
	...
	if(!isGhost())
		IOManager::getInstance()->updateOnlineStatus(guid, false);
	...
	{
		if(IOManager::getInstance()->savePlayer(this))
			saved = true;
#ifdef __DEBUG__
		else
...
void Player::dropCorpse(DeathList deathList)
{
	...
	else
	{
		...
		if(g_config.getBool(ConfigManager::DEATH_LIST))
			IOManager::getInstance()->playerDeath(this, deathList);
	}
}
...
Skulls_t Player::getSkullType(const Creature* creature) const
{
	...
	if(player && player->getSkull() == SKULL_NONE)
	{
		...
		if((player == this || (skull != SKULL_NONE))
...
bool Player::addUnjustifiedKill(const Player* attacked, bool countNow)
{
	...
	IOManager::getInstance()->getUnjustifiedDates(guid, dateList, now);
	if(countNow)
	...
	if(!g_config.getBool(ConfigManager::USE_BLACK_SKULL))
	{
		...
		g_game.addMagicEffect(getPosition(), NM_ME_WRAPS_GREEN);
...
void Player::manageAccount(const std::string &text)
{
	...
	switch(accountManager)
	{
		case MANAGER_NAMELOCK:
		{
			if(!talkState[1])
			{
				...
				if(managerString.length() < g_config.getNumber(ConfigManager::MIN_PLAYER_NAME))
				...
				else if(managerString.length() > g_config.getNumber(ConfigManager::MAX_PLAYER_NAME))
				...
				else if(IOManager::getInstance()->playerExists(managerString, true))
			...
			{
				if(!IOManager::getInstance()->playerExists(managerString, true))
				{
					uint32_t tmp;
					if(IOManager::getInstance()->getGuidByName(tmp, managerString2) &&
						IOManager::getInstance()->changeName(tmp, managerString, managerString2) &&
		...
		{
			Account account = IOManager::getInstance()->loadAccount(managerNumber);
			...
			{
				switch(IOManager::getInstance()->deleteCharacter(managerNumber, managerString))
				{
			...
			else if(talkState[4])
			{
				std::string tmp = text;
				trimString(tmp);
				if(tmp.length() < g_config.getNumber(ConfigManager::MIN_MANAGER_PASSWORD))
			...
			else if(checkText(text, "yes") && talkState[5])
			{
				...
				IOManager::getInstance()->setPassword(managerNumber, managerString);
			...
			else if(checkText(text, "yes") && talkState[9])
			{
				...
				else if(!IOManager::getInstance()->playerExists(managerString, true))
				{
					...
					if(IOManager::getInstance()->createCharacter(managerNumber, managerString, managerNumber2, (uint16_t)managerSex))
			...
			else if(checkText(text, "yes") && talkState[10])
			{
				...
				else
				{
					...
					IOManager::getInstance()->setRecoveryKey(managerNumber, managerString);
		...
		case MANAGER_NEW:
		{
			...
			else if(checkText(text, "yes") && talkState[3])
			{
				if(g_config.getBool(ConfigManager::GENERATE_ACCOUNT_NUMBER))
				{
					...
					while(IOManager::getInstance()->accountNameExists(managerChar));

					uint32_t id = (uint32_t)IOManager::getInstance()->createAccount(managerChar, managerString);
					if(id)
					{
			...
			else if(talkState[4])
			{
				std::string tmp = text;
				trimString(tmp);
				if(tmp.length() < g_config.getNumber(ConfigManager::MIN_ACCNAME))
				...
				else if(tmp.length() > g_config.getNumber(ConfigManager::MAX_ACCNAME))
			...
			{
				if(!IOManager::getInstance()->accountNameExists(managerChar))
				{
					uint32_t id = (uint32_t)IOManager::getInstance()->createAccount(managerChar, managerString);
					if(id)
					{
			...
			{
				managerString = text;
				if(IOManager::getInstance()->getAccountId(managerString, (uint32_t&)managerNumber))
				{
					talkState[7] = false;
					talkState[8] = true;
			...
			{
				managerString2 = text;
				if(IOManager::getInstance()->validRecoveryKey(managerNumber, managerString2) && managerString2 != "0")
				{
					sprintf(managerChar, "%s%d", g_config.getString(ConfigManager::SERVER_NAME).c_str(), random_range(100, 999));
					IOManager::getInstance()->setPassword(managerNumber, managerChar);
	...
	if(!noSwap)
		sendTextMessage(MSG_STATUS_CONSOLE_TEAL, "Hint: Type 'account' to manage your account and if you want to start over then type 'cancel'.");
}
...
bool Player::transferMoneyTo(const std::string& name, uint64_t amount)
{
	...
	if(target->isVirtual())
	{
		IOManager::getInstance()->savePlayer(target);
		delete target;
	}
...
void Player::setMounted(bool mounting)
{
	...
	if(g_config.getBool(ConfigManager::MOUNTS_PREMIUM) && !isPremium())
