...
void ProtocolAdmin::adminCommandSendMail(const std::string& xmlData)
{
	...
	{
		if(IOManager::getInstance()->playerMail(NULL, name, depotId, mailItem))
		{
...
Item* Admin::createMail(const std::string xmlData, std::string& name, uint32_t& depotId)
{
	...
	else if(!IOManager::getInstance()->getDefaultTownByName(name, depotId)) //use the players default town
		return false;
