...
void Creature::onTickCondition(ConditionType_t type, int32_t, bool& _remove)
{
	if(const MagicField* field = getTile()->getFieldItem())
	{
		switch(type)
		{
			case CONDITION_FIRE: _remove = (field->getCombatType() != COMBAT_FIREDAMAGE); break;
			case CONDITION_ENERGY: _remove = (field->getCombatType() != COMBAT_ENERGYDAMAGE); break;
			case CONDITION_POISON: _remove = (field->getCombatType() != COMBAT_EARTHDAMAGE); break;
			case CONDITION_DROWN: _remove = (field->getCombatType() != COMBAT_DROWNDAMAGE); break;
			case CONDITION_BLEEDING: _remove = (field->getCombatType() != COMBAT_BLEEDDAMAGE); break;
			default:
				break;
		}
	}
