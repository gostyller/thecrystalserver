...
bool ExceptionHandler::LoadMap()
{
	...
	FILE* input = fopen("thecrystalserver.map", "r");
	if(!input)
	{
		MessageBoxA(NULL, "Failed loading symbols, thecrystalserver.map file not found.", "Error", MB_OK | MB_ICONERROR);
		std::clog << "Failed loading symbols, thecrystalserver.map file not found. " << std::endl;
		exit(1);
		return false;
	}
