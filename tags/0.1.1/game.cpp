		...
		switch(newState)
		{
			case GAMESTATE_INIT:
			{
				...
				if(g_config.getBool(ConfigManager::INIT_PREMIUM_UPDATE))
					IOManager::getInstance()->updatePremiumDays();
...
bool Game::isSwimmingPool(Item* item, const Tile* tile, bool checkProtection) const
{
	...
	return trashHolder && trashHolder->getEffect() == NM_ME_LOSE_ENERGY && (!checkProtection
...
Player* Game::getPlayerByNameEx(const std::string& s)
{
	...
	std::string name = s;
	if(!IOManager::getInstance()->playerExists(name))
		return NULL;

	player = new Player(name, NULL);
	if(IOManager::getInstance()->loadPlayer(player, name))
		return player;
...
Player* Game::getPlayerByGuidEx(uint32_t guid)
{
	...
	std::string name;
	if(!IOManager::getInstance()->getNameByGuid(guid, name))
		return NULL;
...
bool Game::playerOpenPrivateChannel(uint32_t playerId, std::string& receiver)
{
	...
	if(IOManager::getInstance()->playerExists(receiver))
...
bool Game::playerRequestAddVip(uint32_t playerId, const std::string& vipName)
{
	...
	if(!IOManager::getInstance()->getGuidByNameEx(guid, specialVip, name))
	{
...
bool Game::combatChangeHealth(CombatType_t combatType, Creature* attacker, Creature* target, int32_t healthChange,
	MagicEffect_t hitEffect/* = NM_ME_UNKNOWN*/, Color_t hitColor/* = COLOR_UNKNOWN*/, bool force/* = false*/)
{
	const Position& targetPos = target->getPosition();
	if(healthChange > 0)
	{
			...
			(g_config.getBool(ConfigManager::SHOW_HEALING_DAMAGE_MONSTER) || !target->getMonster()))
		{
			...
			if(combatType != COMBAT_HEALING)
				addMagicEffect(list, targetPos, NM_ME_WRAPS_BLUE);
	...
	else
	{
		...
		if(damage != 0)
		{
			...
			if(damage > 0)
			{
				...
				MagicEffect_t magicEffect = NM_ME_NONE;
				switch(combatType)
				{
					case COMBAT_PHYSICALDAMAGE:
					{
						...
						switch(target->getRace())
						{
							case RACE_VENOM:
								...
								magicEffect = NM_ME_POISON;
								splash = Item::CreateItem(ITEM_SMALLSPLASH, FLUID_GREEN);
								break;

							case RACE_BLOOD:
								textColor = COLOR_RED;
								magicEffect = NM_ME_DRAW_BLOOD;
								splash = Item::CreateItem(ITEM_SMALLSPLASH, FLUID_BLOOD);
								break;

							case RACE_UNDEAD:
								textColor = COLOR_GREY;
								magicEffect = NM_ME_HIT_AREA;
								break;
							...
							case RACE_ENERGY:
								...
								magicEffect = NM_ME_PURPLEENERGY;
								break;
					...
					{
						textColor = COLOR_PURPLE;
						magicEffect = NM_ME_ENERGY_DAMAGE;
						break;
					}
					...
					{
						textColor = COLOR_LIGHTGREEN;
						magicEffect = NM_ME_POISON_RINGS;
						break;
					}
				...
				if(hitEffect != NM_ME_UNKNOWN)
					magicEffect = hitEffect;

				if(hitColor != COLOR_UNKNOWN)
					textColor = hitColor;

				if(textColor < COLOR_NONE && magicEffect < NM_ME_NONE)
				{
					char buffer[20];
	...
	std::string comment, std::string statement, uint32_t statementId, bool ipBanishment)
{
	...
	Account account = IOManager::getInstance()->loadAccount(target->getAccount(), true);
	enum KickAction {
		NONE = 1,
		KICK = 2,
		FULL_KICK = 3,
	...
	else if(kickAction > NONE)
	{
		...
		addMagicEffect(target->getPosition(), NM_ME_WRAPS_GREEN);
	...
	switch(reload)
	{
		...
		case RELOAD_MOUNTS:
		{
			if(Mounts::getInstance()->reload())
				...
				std::clog << "[Error - Game::reloadInfo] Failed to reload mounts." << std::endl;
...
bool Game::playerChangeMountStatus(uint32_t playerId, bool status)
{
	...
	if((OTSYS_TIME() - player->getLastMountAction()) < g_config.getNumber(ConfigManager::MOUNT_COOLDOWN))
	{
		std::stringstream ss;
