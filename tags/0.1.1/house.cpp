...
void House::updateDoorDescription(std::string _name/* = ""*/)
{
	...
	if(owner)
	{
		if(isGuild())
			...
			IOManager::getInstance()->getNameByGuid(owner, _name);
...
void House::removePlayer(Player* player, bool ignoreRights)
{
	...
	{
		g_game.addMagicEffect(curPos, NM_ME_POFF);
...
bool AccessList::addPlayer(std::string& name)
{
	std::string tmp = name;
	uint32_t guid;
	if(!IOManager::getInstance()->getGuidByName(guid, tmp) || playerList.find(guid) != playerList.end())
		return false;
...
bool Houses::payHouse(House* house, time_t _time, uint32_t bid)
{
	...
	std::string name;
	if(!IOManager::getInstance()->getNameByGuid(owner, name))
	{
		house->setOwnerEx(0, true);
		return false;
	}
...
{
	Account account = IOManager::getInstance()->loadAccount(accId);
	uint32_t guid, count = 0;
	for(Characters::iterator it = account.charList.begin(); it != account.charList.end(); ++it)
	{
#ifndef __LOGIN_SERVER__
		if(IOManager::getInstance()->getGuidByName(guid, (*it)) && getHouseByPlayerId(guid))
#else
		if(IOManager::getInstance()->getGuidByName(guid, (std::string&)it->first) && getHouseByPlayerId(guid))
#endif
			count++;
	}
