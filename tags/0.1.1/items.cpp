...
int32_t Items::loadFromOtb(std::string file)
{
	...
	else if((Items::dwMinorVersion < CLIENT_VERSION_860) || (Items::dwMinorVersion > CLIENT_VERSION_870))
	{
...
void Items::parseItemNode(xmlNodePtr itemNode, uint32_t id)
{
	...
	while(itemAttributesNode)
	{
		if(readXMLString(itemAttributesNode, "key", strValue))
		{
			...
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
					if(g_config.getBool(ConfigManager::ENABLE_CAP))
			...
			else if(tmpStrValue == "absorbpercentmagic")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
				{
					...
					it.abilities.absorb[COMBAT_BLEEDDAMAGE] += intValue;
				}
			}
			...
			else if(tmpStrValue == "absorbpercentbleed")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
					it.abilities.absorb[COMBAT_BLEEDDAMAGE] += intValue;
			}
			...
			else if(tmpStrValue == "reflectpercentmagic")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
				{
					...
					it.abilities.reflect[REFLECT_PERCENT][COMBAT_BLEEDDAMAGE] += intValue;
				}
			}
			...
			else if(tmpStrValue == "reflectpercentbleed")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
					it.abilities.reflect[REFLECT_PERCENT][COMBAT_BLEEDDAMAGE] += intValue;
			}
			...
			else if(tmpStrValue == "reflectchancemagic")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
				{
					...
					it.abilities.reflect[REFLECT_CHANCE][COMBAT_BLEEDDAMAGE] += intValue;
				}
			}
			...
			else if(tmpStrValue == "reflectchancebleed")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
					it.abilities.reflect[REFLECT_CHANCE][COMBAT_BLEEDDAMAGE] += intValue;
			}
			...
			else if(tmpStrValue == "suppressbleed")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue) && intValue != 0)
			...
			else if(tmpStrValue == "elementbleed")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
				{
					it.abilities.elementDamage = intValue;
					it.abilities.elementType = COMBAT_BLEEDDAMAGE;
				}
			}
