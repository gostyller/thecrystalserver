...
bool Chat::talkToChannel(Player* player, SpeakClasses type, const std::string& text, uint16_t channelId)
{
	...
	else if(text.substr(1, 6) == "invite")
	{
		if(player->getGuildLevel() > GUILDLEVEL_MEMBER)
		{
			if(text.length() > 7)
			{
				...
				else if(IOManager::getInstance()->playerExists(param))
				{
					uint32_t guid;
					IOManager::getInstance()->getGuidByName(guid, param);
				...
				if(IOGuild::getInstance()->hasGuild(guid))
				{
					if(player->getGuildId() == IOGuild::getInstance()->getGuildId(guid))
					{
						if(text[2] == 'r')
						{
							...
							{
								if(IOManager::getInstance()->isPremium(guid))
								{
						...
						else if(text[2] == 'a')
						{
							...
							{
								const uint32_t levelToFormGuild = g_config.getNumber(ConfigManager::LEVEL_TO_FORM_GUILD);
								if(IOManager::getInstance()->getLevel(guid) >= levelToFormGuild)
								{
						...
						else
						{
							...
							channel->talk(player, SPEAK_CHANNEL_W, buffer);
							IOManager::getInstance()->resetGuildInformation(guid);
						}
					}
				}
				else
	...
	else if(text.substr(1, 4) == "nick" && text.length() > 5)
	{
		...
		if(params.size() >= 2)
		{
			...
			else if(IOManager::getInstance()->playerExists(param1))
			{
				uint32_t guid;
				IOManager::getInstance()->getGuidByName(guid, (std::string&)param1);
				if(IOGuild::getInstance()->hasGuild(guid))
				{
					if(param2.length() > 2)
					{
