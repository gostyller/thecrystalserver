...
bool ConfigManager::load()
{
	...
	if(!m_loaded) //info that must be loaded one time- unless we reset the modules involved
	{
		...
		m_confString[SQL_DB] = getGlobalString("sqlDatabase", "thecrystalserver");
		m_confString[SQL_USER] = getGlobalString("sqlUser", "root");
		m_confString[SQL_PASS] = getGlobalString("sqlPass", "");
		m_confString[SQL_FILE] = getGlobalString("sqlFile", "thecrystalserver.s3db");
		...
		m_confString[MAP_NAME] = getGlobalString("mapName", "SadTeam.otbm");
	...
	m_confString[SERVER_NAME] = getGlobalString("serverName", "Crystal Server");
	...
	m_confNumber[COMBAT_EXHAUSTED] = getGlobalNumber("fightexhausted", 2000);
	m_confNumber[HEAL_EXHAUSTED] = getGlobalNumber("healexhausted", 1000);
	...
	m_confBool[EXP_COLOR_RANDOM] = getGlobalBool("useRandomExperienceColor", false);
	...
	m_confBool[ENABLE_COOLDOWN] = getGlobalBool("useCooldowns", true);
	...
	m_confNumber[MAX_DEPOT_LIMIT_FREE] = getGlobalNumber("depotItemsFreeLimit", 20);
	m_confNumber[MAX_DEPOT_LIMIT_PREMIUM] = getGlobalNumber("depotItemstPremiumLimit", 100);
	...
	m_confBool[SAVE_GAME_DATA] = getGlobalBool("saveGameData", true);
	m_confBool[ENABLE_CAP] = getGlobalBool("useCapSystem", true);
	m_confBool[ALLOW_MOUNTS] = getGlobalBool("useMounts", true);
	m_confBool[DISMOUNT_PLAYER_IN_PZ] = getGlobalBool("dismountPlayerInPz", true);
	m_confBool[MOUNTS_PREMIUM] = getGlobalBool("mountsOnlyPremium", true);
	m_confNumber[MOUNT_COOLDOWN] = getGlobalNumber("mountCooldown", 2000);
	m_confBool[USE_MC] = getGlobalBool("useMultiClient", false);
