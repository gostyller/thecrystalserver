...
function isAchievementSecret(ach) --ach = achievement name or id
	local ach = type(ach) == "string" and getAchievementInfoByName(ach) or getAchievementInfo(tonumber(ach))
	return ach.secret or false
end
 
function getPlayerAchievements(cid, name) --if name then return achievements names instead of achievements ids
    	local done = {}
...
function playerHasAchievement(cid, ach) --ach = achievement name or id
	local ach = type(ach) == "string" and getAchievementInfoByName(ach) or getAchievementInfo(tonumber(ach))
...
function getPlayerSecretAchievements(cid, name) --if name then return achievements names instead of achievements ids
	local done = {}
    	local name = name or false
    	for i = ACHIEVEMENT_FIRST, ACHIEVEMENT_LAST do
		if getCreatureStorage(cid, ACHIEVEMENT_BASE + i) >= getAchievementInfo(i).value then
...
function doPlayerAddAchievement(cid, ach) --ach = achievement name or id
...
function doPlayerRemoveAchievement(cid, ach) --ach = achievement name or id
