...
Account IOManager::loadAccount(uint32_t accountId, bool preLoad/* = false*/)
{
	...
	do
	{
		...
		if(GameServer* server = GameServers::getInstance()->getServerById(result->getDataInt("world_id")))
			...
			std::clog << "[Warning - IOManager::loadAccount] Invalid server for player '" << ss << "'." << std::endl;
#endif
	}
	while(result->next());
	result->free();
...
bool IOManager::loadPlayer(Player* player, const std::string& name, bool preLoad /*= false*/)
{
	...
	<< "`maglevel`, `health`, `healthmax`, `blessings`, `pvp_blessing`, `mana`, `manamax`, `manaspent`, `soul`, `lookbody`, "
			...
			if(pid >= 0 && pid < 100)
			{
				if(Container* c = item->getContainer())
				{
					if(Depot* depot = c->getDepot())
						...
						std::clog << "[Error - IOManager::loadPlayer] Cannot load depot " << pid << " for player " << name << std::endl;
				}
				else
					std::clog << "[Error - IOManager::loadPlayer] Cannot load depot " << pid << " for player " << name << std::endl;
			}
	...
	do
	{
		...
		if(Item* item = Item::CreateItem(result->getDataInt("itemtype"), result->getDataInt("count")))
		{
			if(!item->unserializeAttr(propStream))
				std::clog << "[Warning - IOManager::loadItems] Unserialize error for item with id " << item->getID() << std::endl;
...
bool IOManager::savePlayer(Player* player, bool preSave/* = true*/, bool shallow/* = false*/)
{
	...
	if(!player->isSaving() || !g_config.getBool(ConfigManager::SAVE_GAME_DATA))
	{
