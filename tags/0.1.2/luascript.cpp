...
void LuaInterface::registerFunctions()
{
	...
	//doAccountSave(accountId)
	lua_register(m_luaState, "doAccountSave", LuaInterface::luaDoAccountSave);
	...
	//isNpcName(name)
	lua_register(m_luaState, "isNpcName", LuaInterface::luaIsNpcName);
	
	//isMonsterName(name)
	lua_register(m_luaState, "isMonsterName", LuaInterface::luaIsMonsterName);
	
	//getHouseTilesSize(houseid)
	lua_register(m_luaState, "getHouseTilesCount", LuaInterface::luaGetHouseTilesCount);

	//getHouseDoorCount(houseid)
	lua_register(m_luaState, "getHouseDoorCount", LuaInterface::luaGetHouseDoorCount);

	//getHouseBedCount(houseid)
	lua_register(m_luaState, "getHouseBedCount", LuaInterface::luaGetHouseBedCount);
...
int32_t LuaInterface::luaDoCreatureAddHealth(lua_State* L)
{
	...
	MagicEffect_t hitEffect = NM_ME_UNKNOWN;
	if(params > 2)
...
{
	//doPlayerSendToChannel(cid, targetId, SpeakClasses, message, channel)
	ScriptEnviroment* env = getEnv();
	if(lua_gettop(L) > 5)
		popNumber(L);
...
int32_t LuaInterface::luaGetPlayerGUIDByName(lua_State* L)
{
	...
	else if(IOManager::getInstance()->getGuidByName(guid, name, multiworld))
...
int32_t LuaInterface::luaGetPlayerNameByGUID(lua_State* L)
{
	...
	std::string name;
	if(!IOManager::getInstance()->getNameByGuid(guid, name, multiworld))
	{
		if(displayError)
			errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
...
int32_t LuaInterface::luaDoPlayerChangeName(lua_State* L)
{
	...
	uint32_t guid = popNumber(L);
	if(IOManager::getInstance()->changeName(guid, newName, oldName))
	{
...
int32_t LuaInterface::luaGetIpByName(lua_State* L)
{
	...
	if(Player* player = g_game.getPlayerByName(name))
		...
		lua_pushnumber(L, IOManager::getInstance()->getLastIPByName(name));
...
int32_t LuaInterface::luaGetAccountIdByName(lua_State* L)
{
	...
	if(Player* player = g_game.getPlayerByName(name))
		...
		lua_pushnumber(L, IOManager::getInstance()->getAccountIdByName(name));
...
int32_t LuaInterface::luaGetAccountByName(lua_State* L)
{
	...
	{
		std::string tmp;
		IOManager::getInstance()->getAccountName(IOManager::getInstance()->getAccountIdByName(name), tmp);
...
int32_t LuaInterface::luaGetAccountIdByAccount(lua_State* L)
{
	//getAccountIdByAccount(accName)
	uint32_t value = 0;
	IOManager::getInstance()->getAccountId(popString(L), value);
	lua_pushnumber(L, value);
	return 1;
}
...
int32_t LuaInterface::luaGetAccountByAccountId(lua_State* L)
{
	//getAccountByAccountId(accId)
	std::string value = 0;
	IOManager::getInstance()->getAccountName(popNumber(L), value);
...
int32_t LuaInterface::luaDoPlayerSendMailByName(lua_State* L)
{
	...
	lua_pushboolean(L, IOManager::getInstance()->playerMail(actor, popString(L), town, item));
	return 1;
}
...
int32_t LuaInterface::luaDoPlayerSave(lua_State* L)
{
	...
	if(Player* player = env->getPlayerByUID(popNumber(L)))
	{
		...
		lua_pushboolean(L, IOManager::getInstance()->savePlayer(player, false, shallow));
	}
	else
	{
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
...
int32_t LuaInterface::luaDoAccountSave(lua_State* L)
{
	//doAccountSave(accountId)
	Account account = IOManager::getInstance()->loadAccount(popNumber(L));
	IOManager::getInstance()->saveAccount(account);
	return 1;
}
...
int32_t LuaInterface::luaDoAddAccountWarnings(lua_State* L)
{
	...
	Account account = IOManager::getInstance()->loadAccount(popNumber(L), true);
...
int32_t LuaInterface::luaIsNpcName(lua_State* L)
{
	//isNpcName(name)
	std::string name = popString(L);
...
int32_t LuaInterface::luaIsMonsterName(lua_State* L)
{
	//isMonsterName(name)
	std::string name = popString(L);
...
int32_t LuaInterface::luaGetHouseTilesCount(lua_State* L)
{
...
int32_t LuaInterface::luaGetHouseDoorCount(lua_State* L)
{
	//getHouseDoorCount(houseid)
	uint32_t houseid = popNumber(L);
	House* house = Houses::getInstance()->getHouse(houseid);
	...
	if(house)
		lua_pushnumber(L, house->getDoorsCount());
	else
	{
		errorEx(getError(LUA_ERROR_HOUSE_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaInterface::luaGetHouseBedCount(lua_State* L)
{
	//getHouseBedCount(houseid)
	uint32_t houseid = popNumber(L);
	House* house = Houses::getInstance()->getHouse(houseid);
