	...
	OperatingSystem_t operatingSystem, uint16_t version, bool gamemaster)
{
	...
	if(!_player || name == "Account Manager" || g_config.getNumber(ConfigManager::ALLOW_CLONES) > (int32_t)players.size())
	{
		...
		player->setID();
		if(!IOManager::getInstance()->loadPlayer(player, name, true))
		{
		...
		if(IOBan::getInstance()->getData(ban) && !player->hasFlag(PlayerFlag_CannotBeBanned))
		{
			...
			if(!ban.adminId)
				...
				IOManager::getInstance()->getNameByGuid(ban.adminId, name_, true);
		...
		if(gamemaster && !player->hasCustomFlag(PlayerCustomFlag_GamemasterPrivileges) && g_config.getBool(ConfigManager::USE_MC))
		{
		...
		if(g_config.getBool(ConfigManager::ONE_PLAYER_ON_ACCOUNT) && !player->isAccountManager() &&
			!IOManager::getInstance()->hasCustomFlag(id, PlayerCustomFlag_CanLoginMultipleCharacters))
		{
			bool found = false;
	...
	if(gamemaster && !_player->hasCustomFlag(PlayerCustomFlag_GamemasterPrivileges) && g_config.getBool(ConfigManager::USE_MC))
	{
		disconnectClient(0x14, "You are not a gamemaster! Turn off the gamemaster mode in your IP changer.");
		return false;
	}
...
bool ProtocolGame::logout(bool displayEffect, bool forceLogout)
{
	...
	if(!player->isRemoved())
	{
		if(!forceLogout)
		{
			if(!IOManager::getInstance()->hasCustomFlag(player->getAccount(), PlayerCustomFlag_CanLogoutAnytime))
			{
...
bool ProtocolGame::parseFirstPacket(NetworkMessage& msg)
{
	...
	uint32_t id = 1;
	if(!IOManager::getInstance()->getAccountId(name, id))
	{
	...
	if(!IOManager::getInstance()->getPassword(id, hash, salt, character) || !encryptTest(salt + password, hash))
	{
		ConnectionManager::getInstance()->addAttempt(getIP(), protocolId, false);
	...
	if(IOBan::getInstance()->getData(ban) && !IOManager::getInstance()->hasFlag(id, PlayerFlag_CannotBeBanned))
	{
		bool deletion = ban.expires < 0;
		std::string name_ = "Automatic ";
		if(!ban.adminId)
		...
		/*	case 0xE7:
			...
			default:
			{
				if(g_config.getBool(ConfigManager::BAN_UNKNOWN_BYTES))
				{
					...
					Account tmp = IOManager::getInstance()->loadAccount(player->getAccount(), true);
					...
					{
						IOManager::getInstance()->saveAccount(tmp);
						player->sendTextMessage(MSG_INFO_DESCR, "You have been banished.");

						g_game.addMagicEffect(player->getPosition(), NM_ME_WRAPS_GREEN);
...
void ProtocolGame::sendAddCreature(const Creature* creature, const Position& pos, uint32_t stackpos)
{
	...
	{
		std::string vipName;
		if(IOManager::getInstance()->getNameByGuid((*it), vipName))
		{
