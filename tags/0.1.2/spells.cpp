...
bool Spell::configureSpell(xmlNodePtr p)
{
	...
	{
		if(isAggressive)
			groupExhaustions[SPELLGROUP_ATTACK] = g_config.getNumber(ConfigManager::COMBAT_EXHAUSTED);
		else
...
bool Spell::checkSpell(Player* player) const
{
	...
	{
		bool useCooldowns = g_config.getBool(ConfigManager::ENABLE_COOLDOWN),
...
bool InstantSpell::Illusion(const InstantSpell*, Creature* creature, const std::string& param)
{
	...
	if(ret == RET_NOERROR)
	{
		g_game.addMagicEffect(player->getPosition(), NM_ME_WRAPS_RED);
		return true;
	}
